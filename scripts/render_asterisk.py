...
{rec_inbound_lines.rstrip()}
 ...
 same => n(ava-readiness),Set(AVA_STASIS_READY=${{STAT(e,{data_dir / 'runtime/ava-stasis-ready'})}})
 same => n,GotoIf($["${{AVA_STASIS_READY}}"="1"]?ava-stasis)
 same => n,Set(AVA_STASIS_ATTEMPTS=$[${{AVA_STASIS_ATTEMPTS}}+1])
 same => n,GotoIf($[${{AVA_STASIS_ATTEMPTS}}=1]?ava-wait-message)
 same => n,GotoIf($[${{AVA_STASIS_ATTEMPTS}}<45]?ava-wait)
 ...
 same => n(ava-stasis),Stasis({safe(env('AVA_STASIS_APP','asterisk-ai-voice-agent'), r'[A-Za-z0-9_.-]+', 'Stasis app', False)})
 same => n,GotoIf($["${{STASISSTATUS}}"="FAILED"]?ava-stasis-failed)
 same => n,Hangup()
 same => n(ava-stasis-failed),Set(AVA_STASIS_ATTEMPTS=$[${{AVA_STASIS_ATTEMPTS}}+1])
 same => n,GotoIf($[${{AVA_STASIS_ATTEMPTS}}<45]?ava-stasis-retry)
 same => n,Goto(provider-failure)
...
{rec_outbound_lines.rstrip()}
 ...
 same => n,GotoIf($["${{STASISSTATUS}}"="FAILED"]?outbound-provider-failure)
 same => n,Hangup()
